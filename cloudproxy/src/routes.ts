...
async function resolveChallenge(ctx: RequestContext, { url, userAgent, maxTimeout, proxy, download, returnOnlyCookies, apiKey, sitekey }: BaseRequestAPICall, page: Page): Promise<ChallengeResolutionT | void> {
        ...
        if (challengeForm) {
          ...
          try {
            ...
            if (err instanceof TimeoutError) {
              log.debug(`No '#challenge-form' element detected.`)
            }
          }
